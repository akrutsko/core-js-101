...
function getComposition(f, g) {
  return function composed(param) {
    return f(g(param));
  };
}
...
function getPolynom(...args) {
  const [c0, c1, c2] = args.reverse();
  return function polynomed(x) {
    let result = null;
    if (c0) result = c0;
    if (c1) result += c1 * x;
    if (c2) result += c2 * x ** 2;
